...
async function createFergusEnquiry(
  ...
  // Strip any leading BOM (U+FEFF = 65279) that PowerShell piping can introduce
  ...
  // Parse address1 from the full address string (everything before the first comma)
...
// ─── Notification email to Devon ─────────────────────────────────────────────
function devonHtml(name: string, phone: string, email: string, address: string, details: string) {
  return `
<!DOCTYPE html>
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
      ...
      <tr><td style="background:#1B4332;border-radius:6px 6px 0 0;padding:28px 36px">
        ...
        <p style="margin:6px 0 0;font-family:Arial,sans-serif;font-size:12px;color:rgba(255,255,255,0.5)">Submitted via greenworx.co.nz</p>
      </td></tr>
      ...
      <tr><td style="background:#111f17;border-radius:0 0 6px 6px;padding:18px 36px">
        <p style="margin:0;font-family:Arial,sans-serif;font-size:11px;color:rgba(255,255,255,0.35);text-align:center">Greenworx Landscaping · Unit 2, 49 Mt Wellington Highway, Auckland</p>
      </td></tr>
          ...
          <tr>
            <td>
              <p style="margin:0;font-family:Arial,sans-serif;font-size:11px;color:rgba(255,255,255,0.35)">
                &copy; 2026 Greenworx Landscaping · Unit 2, 49 Mt Wellington Highway, Auckland
              </p>
            </td>
            <td align="right">
...
export async function POST(req: Request) {
  try {
    ...
    // The notification to Devon is critical — fail if it didn't send.
    if (notification.status === "rejected") {
      console.error("Notification email failed:", notification.reason);
      return NextResponse.json({ error: "Failed to send email." }, { status: 500 });
    }
    if (notification.value.error) {
      console.error("Notification Resend error:", JSON.stringify(notification.value.error));
      return NextResponse.json({ error: "Failed to send email." }, { status: 500 });
    }

    // Log confirmation result but don't fail the request over it.
    if (confirmation.status === "rejected") {
      console.warn("Confirmation email failed (non-fatal):", confirmation.reason);
    } else if (confirmation.value.error) {
      console.warn("Confirmation Resend error (non-fatal):", JSON.stringify(confirmation.value.error));
    }

    // Create Fergus enquiry — non-fatal, never blocks the user response.
